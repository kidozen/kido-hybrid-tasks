 ...
 * @param {string} marketplace url
 * @param {object} opts
 *  - {fn} onTokenExpired gets called when the authentication token is expired
 */
...
var Kido = function (name, marketplace, opts) {
    ...
    this.authenticate = function (user, pass, prov) {
        ...
        this.token = this.authConfig.pipe(function (config) {
            ...
            if (!ip.protocol || !protocols[ip.protocol.toLowerCase()])
                throw new Error("Invalid Identity Provider Protocol");

            var getToken = protocols[ip.protocol.toLowerCase()];
            return getToken({
                user: user,
                pass: pass,
